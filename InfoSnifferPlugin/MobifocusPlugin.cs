...
{
    public class MobifocusPlugin : IPlugin
    {
        public void Receive(DataSet data,string file)
        {
            ...
            for (int i = table.Rows.Count - 1; i >= 0; i--)
            {
                try
                {
                    ...
                    //这里保存一部份内容作为摘要使用
                    post.Content = StringUtil.GetByteSubString(StringUtil.ClearHtmlCode(content), 1000);

                    //不再保存内容到数据库里，而是链接到xml文件，这样就可以大大的节省数据库空间了。
                    XmlNode node = post.PluginDatas.CreateElement("contentplugin");
                    node.InnerXml = string.Format("<xmlfile>{0}</xmlfile><id>{1}</id>", file, row["ID"]);
